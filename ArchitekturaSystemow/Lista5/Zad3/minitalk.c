...
int main() {
    ...
    miniserver = socket(AF_INET , SOCK_STREAM , 0);
    if (miniserver == -1) {
        printf("Socket error\n");
        exit(EXIT_FAILURE);
    }
    ...
    if(bind(miniserver, (struct sockaddr *)&server, sizeof(server)) < 0) {
        printf("Bind error\n");
        return 1;
    }
    printf("Bind ok\n");
    listen(miniserver, 10);
/*
    root = newNode(server_sockfd, inet_ntoa(server_info.sin_addr));
    now = root;
*/
    while (1) {
        new_user = accept(miniserver, (struct sockaddr*) &client, (socklen_t*)&c);
        printf("Dolaczono");
/*
