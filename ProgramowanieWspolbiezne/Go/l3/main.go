...
var multMachines []machine
var newService chan service
...
func main() {
	...
	servicesDone := make(chan service)
	serviceRequests := make(chan serviceRequest)
	go machineService(newService, servicesDone, serviceRequests)
	for i:=0; i < serviceWorkersNumber; i++ {
		go serviceWorker(serviceRequests, servicesDone)
	}
